  ...
  return (
    <div className="space-y-3">
      {reminders.map((r, i) => (
        <div
          key={i}
          className="bg-surface border border-outline-variant border-l-4 border-l-primary rounded-xl p-3 md:p-4 flex items-center gap-3 md:gap-4"
        >
          <Icon icon="priority_high" size="lg" className="text-primary-fixed-dim" />
      ...
      {!dismissed && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          {!showForm ? (
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-2 w-full bg-surface rounded-xl border border-outline-variant p-3">
              ...
              <button onClick={addReminder} className="bg-primary text-on-primary px-5 py-2 rounded-lg font-label-md transition-all min-h-[44px]">Save</button>
              <button onClick={() => setShowForm(false)} className="px-4 py-2 text-secondary font-label-md hover:text-primary min-h-[44px]">Cancel</button>
            </div>
          )}
        </div>
      )}
